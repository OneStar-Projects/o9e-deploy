...
> | 告警规则里的 PromQL | 按规则所属组收窄 | ❌ **未做**；09-01 改用「收回 Standard 写权」兜底，见 §10 |
>
> `Center.RegionIsolation.Enable` 现场仍是 **false**，指标层隔离**没有生效**。
> 打开前的完整验证矩阵（含「用户自写 `ident` 不能被放大成全部」这条关键回归）
> 在计划文件里，不重复。
>
> **更正（09-01）**：上面「两批修复的行为验证都还没跑过」的警告**已作废**。
> 09-01 用临时账号（Standard + 区域用户组）跑完了 B 类越权验证，
> 上表「08-30」一列的 ✅ 现在是**实测结论**，不再只是代码审阅。
> 明细见 ops-log `2026-09-01` 第一节，其中三条值得单独记住：
>
> - **告警事件类判权必须先确认 id 当前存在** —— 告警会自动恢复，
>   用已消失的 id 测会拿到 `200 {"dat":[]}`（判权循环没执行）或 `500`，
>   两者都**不是**「未拦截」，而是**没测到**。
> - **`DELETE /busi-group/:id/alert-rules` 跨组返回 200 而不是 403** ——
>   `models.AlertRuleDels` 靠 SQL 的 `AND group_id = ?` 兜底、删 0 行。
>   数据安全，但**看到 200 不等于操作生效**，判断写路径是否被挡必须查数据。
> - `cfgsync/{instances,hosts}?gids=<别人的组>` 返回 `200 []` 是**过滤语义**，
>   不是判权端点，与 `targets?gids=` 的 403 形态不同，别当成漏挡。
>
> 仍未实测的只剩**指标层本身**（`/proxy/1/api/v1/query` 那两行）——
> 它们要等 `Enable` 打开后才能测。
...
| 3 | 告警规则给区域 `ro` 还是 `rw` | 权限设计 | **已定（09-01）：走 A，收回写权，即 `ro`**，见下 |
...
> ```
>
> Standard 现只剩 `/alert-rules`、`/alert-rules-built-in` 两条只读。影响面为零
> （全站仅 `admin`/Admin 与测试账号 `alert`/Standard）。回滚 = 把三行 INSERT 回去。
>
> **这条缺口现在的状态是「收窄到只有 admin 能碰」，不是「修好了」** —— 根因
> （求值无用户上下文、PromQL 不注入）仍在。接第二区域且区域方要自建规则时，
> 必须回来做 B，届时先定「非机器组规则怎么办」。详见 ops-log 2026-09-01 第二节。
