...
#   B. group-config  给该组写对外端口 proxy_controller_port=17001 / ingester_port=17002(存 server MySQL)
...
# ============ B. 写 agent-group-config：对齐端口 ============
GC="$(mktemp)"; trap 'rm -f "$GC"' EXIT
cat > "$GC" <<EOF
global:
  communication:
    proxy_controller_port: ${PROXY_CONTROLLER_PORT}
    ingester_port: ${INGESTER_PORT}
EOF
log "对 group ${AGENT_GROUP_ID} 写端口 control=${PROXY_CONTROLLER_PORT} data=${INGESTER_PORT}"
if ctl agent-group-config update "$AGENT_GROUP_ID" -f "$GC" 2>/dev/null; then
    log "已 update 现有 group-config"
else
    ctl agent-group-config create "$AGENT_GROUP_ID" -f "$GC"
    log "已 create group-config"
fi
log "回读校验："
ctl agent-group-config list "$AGENT_GROUP_ID" -o yaml 2>/dev/null \
    | grep -E "proxy_controller_port|ingester_port" || true
