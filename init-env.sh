...
#   - 设宿主 vm.max_map_count>=262144(ES 硬性要求;能免密 sudo 则自动设,否则打印手动命令)
#
# 设计约束:
...
ensure_es_sysctl() {
    # Elasticsearch 硬性要求宿主 vm.max_map_count >= 262144,否则容器启动即崩。
    # init-env 通常非 root 跑,故:能免密 sudo 就自动设并持久化,不能就打印手动命令。
    local want=262144 cur
    cur="$(cat /proc/sys/vm/max_map_count 2>/dev/null || echo 0)"
    if [ "${cur:-0}" -ge "$want" ] 2>/dev/null; then
        echo "[init-env] vm.max_map_count=${cur} 已满足 ES 要求(≥${want})"
        return 0
    fi
    if sudo -n sysctl -w "vm.max_map_count=${want}" >/dev/null 2>&1; then
        echo "vm.max_map_count=${want}" | sudo -n tee /etc/sysctl.d/99-elasticsearch.conf >/dev/null 2>&1 || true
        echo "[init-env] 已设 vm.max_map_count=${want}(并持久化到 /etc/sysctl.d/99-elasticsearch.conf)"
    else
        echo "[init-env] WARN: vm.max_map_count=${cur} < ${want},ES 会启动失败。请手动执行:" >&2
        echo "[init-env]   sudo sysctl -w vm.max_map_count=${want}" >&2
        echo "[init-env]   echo 'vm.max_map_count=${want}' | sudo tee /etc/sysctl.d/99-elasticsearch.conf" >&2
    fi
}
...
ELASTIC_PWD="$(gen_pwd)"          # elastic 超级用户密码(Logstash + n9e ES 数据源做 basic auth)
...
/^ELASTIC_PASSWORD=/          { print "ELASTIC_PASSWORD=" elastic; next }
{ print }
' "${SRC}" > "${DST}"
...
echo "[init-env]   7 个密码/token 已替换为 32 字符强随机(A-Za-z0-9):"
...
echo "[init-env]     ELASTIC_PASSWORD (elastic 超级用户,Logstash + n9e ES 数据源用)"
