...
# 步骤 2: 清理旧组件
###############################################################################
log_step "步骤 2/9: 清理 podman / 旧 containerd"
...
# 步骤 3: 配置 docker-ce 源
###############################################################################
log_step "步骤 3/9: 配置 docker-ce 源"
...
###############################################################################
log_step "步骤 4/9: 安装 container-selinux"
...
# 步骤 5: 安装 Docker
###############################################################################
log_step "步骤 5/9: 安装 Docker CE $DOCKER_VERSION"
...
###############################################################################
# 步骤 9: 内核参数(Elasticsearch 需要)
###############################################################################
log_step "步骤 9/9: 内核参数调优(Elasticsearch)"

# ES 8.x 要求 vm.max_map_count >= 262144;麒麟 V10 默认 65530,过低会导致
# elasticsearch 容器 bootstrap 检查失败、启动即退出(healthcheck 永远 unhealthy,
# 依赖它的 logstash 也起不来)。这里写 sysctl.d 持久化 + 立即生效。
SYSCTL_ES_CONF="/etc/sysctl.d/99-elasticsearch.conf"
echo "vm.max_map_count=262144" > "$SYSCTL_ES_CONF"
sysctl -p "$SYSCTL_ES_CONF" > /dev/null 2>&1 || true

CUR_MMC=$(sysctl -n vm.max_map_count 2>/dev/null || echo 0)
if [[ "$CUR_MMC" -ge 262144 ]]; then
    log_ok "vm.max_map_count = $CUR_MMC(已持久化到 $SYSCTL_ES_CONF)"
else
    log_warn "vm.max_map_count 仍为 $CUR_MMC(< 262144),elasticsearch 可能起不来"
fi
